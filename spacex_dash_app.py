...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All':
        ...
        data=filter_df.groupby('class')['Launch Site'].count().reset_index()
        fig = px.pie(data, values='Launch Site', 
        names='class', 
        title='Launch outcome details for site '+ entered_site)
        return fig
...
def get_scatter_plot(entered_site,payload):
    ...
    if entered_site == 'All':
        data=filtered_df[filtered_df["Payload Mass (kg)"]>=payload[0]]
        ...
        # return the outcomes piechart for a selected site
        filter_df= spacex_df[spacex_df["Launch Site"]==entered_site]
        data=filter_df[filter_df["Payload Mass (kg)"]>=payload[0]]
        data=data[data["Payload Mass (kg)"]<=payload[1]]
        fig = px.scatter(data, x='Payload Mass (kg)', 
        y='class',
        color="Booster Version Category", 
